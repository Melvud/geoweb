...
export function PublicHomeView() {
  ...
  const featuredPublications = publicPublications.filter((item) => item.featured);
    ...
    <Fragment>
      ...
      <section className="public-section" style={{ paddingTop: 30 }}>
        ...
        <div className="photo-grid">
          {state.photos.slice(0, 6).map((item) => (
            <button key={item.id} onClick={() => openDetail({ kind: "photo", item })}>
              <div
                style={{
                  ...
                  borderRadius: 12,
                  border: "1px solid var(--line)",
                  background: `repeating-linear-gradient(135deg, ${item.tint}, ${item.tint} 11px, rgba(255,255,255,.16) 11px, rgba(255,255,255,.16) 22px)`,
                  position: "relative",
                }}
              >
                <span
